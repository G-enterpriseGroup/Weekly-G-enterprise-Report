...
# Runs weekly from GitHub Actions or manually from your Mac.
# GitHub-ready version: outputs to ./output and publishes latest report to ./public
...
# Required for GitHub Actions/headless servers.
import matplotlib
matplotlib.use("Agg")
...
# GitHub Actions uses ./output. On your Mac you can override with GHOTRA_OUTPUT_DIR.
BASE_DIR = Path(os.getenv("GHOTRA_OUTPUT_DIR", "output"))
...
OPEN_HTML_AFTER = os.getenv("OPEN_HTML_AFTER", "0") == "1"  # Off in GitHub Actions.
LOOKBACK_DAYS = 420         # Enough data to chart trends and calculate weekly changes.
REQUEST_TIMEOUT = 30
FRED_CONNECT_TIMEOUT = 10
FRED_READ_TIMEOUT = 90
FRED_RETRIES = 3
GDELT_DELAY_SECONDS = 2.0   # Helps reduce GDELT 429 rate-limit errors.

# Wording variation / local AI rewrite settings.
# No API key is used. If Ollama is running locally on your Mac, the script can ask a local model
# to polish sections. If Ollama is not installed/running, the built-in analyst
# variation engine still creates fresh professional wording every run. In GitHub Actions this is off by default.
USE_LOCAL_LLM_REWRITER = os.getenv("USE_LOCAL_LLM_REWRITER", "0") == "1"
...
def fred_bulk_csv_url(series_ids: List[str], start: date, end: date) -> str:
    ids = ",".join(series_ids)
    return (
        "https://fred.stlouisfed.org/graph/fredgraph.csv"
        f"?id={ids}&cosd={start.isoformat()}&coed={end.isoformat()}"
    )

def fred_headers() -> Dict[str, str]:
    return {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) Ghotra-Capital-Weekly-Report/1.0",
        "Accept": "text/csv,text/plain,*/*",
        "Cache-Control": "no-cache",
        "Pragma": "no-cache",
    }

def normalize_fred_series(df: pd.DataFrame, series_id: str) -> pd.DataFrame:
    ...
    raw_values = df[series_id].astype(str).str.strip().replace({".": np.nan, "": np.nan, "nan": np.nan})
    out = pd.DataFrame({
...
def fetch_fred_bulk(series_ids: List[str], start: date, end: date) -> pd.DataFrame:
    """
    Fetch all FRED series in one CSV request.
    This avoids GitHub Actions making 14+ separate calls that can time out.
    """
    ...
    url = fred_bulk_csv_url(unique_ids, start, end)
    headers = fred_headers()

    for attempt in range(1, FRED_RETRIES + 1):
        try:
            print(f"FRED bulk attempt {attempt}/{FRED_RETRIES}: {len(unique_ids)} series")
            r = requests.get(url, headers=headers, timeout=(FRED_CONNECT_TIMEOUT, FRED_READ_TIMEOUT))
            print(f"FRED bulk: HTTP {r.status_code}, bytes={len(r.text or '')}")

            if r.status_code != 200:
                print(f"FRED bulk failed. First 300 chars: {(r.text or '')[:300]!r}")
                time.sleep(3 * attempt)
                continue

            if not r.text.strip():
                print("FRED bulk returned an empty response.")
                time.sleep(3 * attempt)
                continue

            df = pd.read_csv(StringIO(r.text))
            print(f"FRED bulk columns={list(df.columns)}, raw_rows={len(df)}")
            return df

        except Exception as e:
            print(f"FRED bulk exception attempt {attempt}: {type(e).__name__}: {e}")
            time.sleep(3 * attempt)
...
def fetch_one_fred_series(series_id: str, start: date, end: date) -> pd.DataFrame:
    """
    Single-series fallback. Usually not needed because bulk FRED is preferred.
    """
    from io import StringIO
    ...
    for attempt in range(1, FRED_RETRIES + 1):
        try:
            print(f"FRED {series_id} single attempt {attempt}/{FRED_RETRIES}")
            r = requests.get(url, headers=headers, timeout=(FRED_CONNECT_TIMEOUT, FRED_READ_TIMEOUT))
            print(f"FRED {series_id}: HTTP {r.status_code}, bytes={len(r.text or '')}")
            ...
            df = pd.read_csv(StringIO(r.text))
            out = normalize_fred_series(df, series_id)
            print(f"FRED {series_id}: clean_rows={len(out)}")
            return out

        except Exception as e:
            print(f"FRED {series_id} exception attempt {attempt}: {type(e).__name__}: {e}")
            time.sleep(3 * attempt)

    return empty

def fetch_series_with_fallback(cfg: SeriesConfig, start: date, end: date, bulk_df: Optional[pd.DataFrame] = None) -> Tuple[str, pd.DataFrame]:
    """Try the primary series ID, then fallbacks. Uses bulk CSV first."""
    for sid in cfg.series_ids:
        df = normalize_fred_series(bulk_df, sid) if bulk_df is not None and not bulk_df.empty else pd.DataFrame(columns=["date", "value"])
        if not df.empty:
            print(f"Using {sid} for {cfg.name} from FRED bulk: {len(df)} rows")
            return sid, df

    # If bulk did not work or a fallback column was missing, try singles only for this config.
    for sid in cfg.series_ids:
        df = fetch_one_fred_series(sid, start, end)
        if not df.empty:
            print(f"Using {sid} for {cfg.name} from FRED single fallback: {len(df)} rows")
            return sid, df
        print(f"No usable data for {cfg.name} via {sid}")
...
def fetch_all_fred_data(start: date, end: date) -> Tuple[Dict[str, pd.DataFrame], Dict[str, str]]:
    ...
    print(f"FRED pull window: {start.isoformat()} to {end.isoformat()}")
    ...
    bulk_df = fetch_fred_bulk(all_series_ids, start, end)

    for cfg in SERIES:
        sid, df = fetch_series_with_fallback(cfg, start, end, bulk_df=bulk_df)
        if not df.empty:
            data[cfg.name] = df
            used_ids[cfg.name] = sid
        else:
            print(f"WARNING: {cfg.name} has no usable data after bulk + fallback.")

    print(f"FRED series loaded: {len(data)} of {len(SERIES)}")
...
# Stop the workflow instead of publishing a blank report with no dashboard/charts.
if not data:
    raise RuntimeError(
        "No FRED macro data was downloaded. The report would be blank, so publishing was stopped. "
        "Check the FRED HTTP/status logs above in GitHub Actions."
    )

dashboard = build_dashboard(data, used_ids, WEEK_END)
if dashboard.empty:
    raise RuntimeError(
        "FRED data downloaded, but the macro dashboard is empty. "
        "Check date alignment and nearest_value_on_or_before logic."
    )
...
# ----------------------------
# 10) Publish latest report for GitHub Pages
# ----------------------------
# GitHub Pages will serve the files inside ./public.
# This keeps the website URL stable while the report refreshes every Monday.
import shutil

PUBLISH_DIR = Path(os.getenv("PUBLISH_DIR", "public"))
PUBLISH_DIR.mkdir(parents=True, exist_ok=True)

# Disable Jekyll so GitHub Pages serves the HTML/charts exactly as generated.
(PUBLISH_DIR / ".nojekyll").write_text("", encoding="utf-8")

# Copy latest HTML to public/index.html
shutil.copy2(html_path, PUBLISH_DIR / "index.html")

# Copy latest charts and CSV files used by the report.
latest_report_dir = Path(html_path).parent
latest_chart_dir = latest_report_dir / "charts"
latest_data_dir = latest_report_dir / "data"

if latest_chart_dir.exists():
    shutil.copytree(latest_chart_dir, PUBLISH_DIR / "charts", dirs_exist_ok=True)

if latest_data_dir.exists():
    shutil.copytree(latest_data_dir, PUBLISH_DIR / "data", dirs_exist_ok=True)

# Keep one dated archive folder inside the Pages output for reference.
archive_dir = PUBLISH_DIR / "archive" / latest_report_dir.name
archive_dir.parent.mkdir(parents=True, exist_ok=True)
shutil.copytree(latest_report_dir, archive_dir, dirs_exist_ok=True)

print(f"GitHub Pages publish folder ready: {PUBLISH_DIR.resolve()}")
print("Latest report will publish as: index.html")
