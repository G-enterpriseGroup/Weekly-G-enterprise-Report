...
def fetch_one_fred_series(series_id: str, start: date, end: date) -> pd.DataFrame:
    """
    Fetch one FRED series from the public FRED graph CSV endpoint.
    Important: this version prints failures instead of hiding them, so GitHub Actions
    will show exactly why charts/dashboard are missing.
    """
    url = fred_csv_url(series_id, start, end)
    headers = {
        "User-Agent": "Mozilla/5.0 Ghotra-Capital-Weekly-Report/1.0",
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"FRED {series_id} failed. First 300 chars: {(r.text or '')[:300]!r}")
            return empty

        if not r.text.strip():
            print(f"FRED {series_id} returned an empty response.")
            return empty

        from io import StringIO
        df = pd.read_csv(StringIO(r.text))
        print(f"FRED {series_id}: columns={list(df.columns)}, raw_rows={len(df)}")
        ...
        # Convert safely. FRED missing values often come through as a period '.'.
        raw_values = df[value_col].astype(str).str.strip().replace({".": np.nan, "": np.nan})
        out = pd.DataFrame({
